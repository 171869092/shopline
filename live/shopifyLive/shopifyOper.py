...
class shopifyOper:
    ...
    def pullOrder(self):
        ...
        try:
            ...
            if cursor and con:
                self.cursor = cursor
                self.db = con
            # sql = "SELECT * FROM `store` WHERE status = 1 AND id = 4"
            ...
            for data in results:
                ...
                print(strTime)
                print(endTime)
                order = shopify.Order().find(None, None, created_at_min=strTime, created_at_max=endTime,limit=50,status='any')
                # order = shopify.Order().find('2189162053655')
                # order = shopify.Order().find('2187568807959')
                # order = shopify.Order().find('2189236862999')
                datas = []
                print("第一页数据：%s" %(order))
                if not isinstance(order, list):
                    ...
                    while True:
                        if b == 0:
                            for xs in order:
                                datas.append(xs.to_dict())
                        if order.has_next_page():
                            nextOrder = order.next_page()
                            print("下一页数据：%s" % (b))
                            for zz in nextOrder:
                                datas.append(zz.to_dict())
                            b += 1
                            order = nextOrder
                        else:
                            break
    ...
    def checkOrder(self,order_id, table, sku = None):
        ...
        if not sku:
            ...
            sql = "SELECT * FROM `%s` WHERE order_id = %s AND sku = '%s'" % (table,order_id,sku)
        curros.execute(sql)
    ...
    def insertOrder(self,data,storeId,storeName):
        ...
        for i in data:
            ...
            orderFormat['phone'] = i['shipping_address']['phone']
    ...
    def insertOrderShipping(self, data):
        ...
        for i in data:
            ...
            if result:
                continue
            try:
                curros.execute("INSERT INTO `shipping_detail` (`order_id`,`first_name`,`last_name`,`address1`,`address2`,`city`,`zip`,`province`,`country`,`company`,`latitude`,`longitude`,`name`,`country_code`,`province_code`,`phone`) " \
                          "VALUES (%s, %s, %s,%s, %s, %s , %s, %s, %s, %s, %s, %s, %s, %s, %s,%s)",(orderFormat['order_id'], orderFormat['first_name'], orderFormat['last_name'],
                              ...
                              orderFormat['latitude'],orderFormat['longitude'],orderFormat['name'] , orderFormat['country_code'], orderFormat['province_code'],orderFormat['phone']))
                db.commit()
            except Exception as e:
                print(e)
                db.rollback()
    ...
    def exportOrder(self, data,storeId,storeName):
        ...
        try:
            ...
            print(self.paths)
            with open(self.paths + "/order-"+storeName+"%s.csv" % time.strftime("%Y-%m-%d"), 'w+', newline='',encoding='utf-8') as f:
